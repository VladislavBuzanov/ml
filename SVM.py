...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.button == 1:
                if not predicted:
                    create_data(pygame.mouse.get_pos(), color[0])
                    clusters.append(0)
                else:
                    r = model.predict([pygame.mouse.get_pos()])
                    clusters.append(r)
                    create_data(pygame.mouse.get_pos(), color[r[0]])
            elif event.button == 3:
                if not predicted:
                    create_data(pygame.mouse.get_pos(), color[1])
                    clusters.append(1)
                else:
                    r = model.predict([pygame.mouse.get_pos()])
                    clusters.append(r)
